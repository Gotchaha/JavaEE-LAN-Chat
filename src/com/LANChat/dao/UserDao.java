...
public class UserDao implements IUserDao{
    ...
    public void insert(NhManageUser user) {
        ...
        Object[] param = {user.getUser_name(),user.getPassword(),user.getReal_name(),user.getDepartment_id(),user
                .getPosition(),user.getGender(),user.getPhone(),user.getEmail(),user.getDelete_flag(),user.getAddress(),user.getWork_phone(),user.getMemo()};
        try {
            qr.update(sql,param);
        } catch (SQLException e) {
            e.printStackTrace();
